  ...
  methods: {
    ...
    setAtributos(attr) {
      ...
      if(attr.simples){
        ...
        attr.simples.valores.map(function(value) {
          valores.push({
            valor: value.label
          });
        });
      ...
      if(attr.unico){
        //OBTÉM TODOS OS VALORES
        valores = [];
        attr.unico.valores.map(function(value) {
          valores.push({
            valor: value.label
          });
        });
    ...
    setPagina(responsePagina){
      ...
      switch (responsePagina.estrutura.nivel) {
        ...
        case 'page':
          return this.setPaginaInstitucional(responsePagina);
      }
    },
    setPaginaHome(){
      //DADOS DA HOME
    ...
    setPaginaInstitucional(responsePagina){
      //DADOS DA PAGINA NÃO ENCONTRADA
      this.wapstore.pagina.nome      = responsePagina.estrutura.seo.title;
      this.wapstore.pagina.secao     = "pagina-institucional";
      this.wapstore.pagina.subsecao  = "institucionais";
      this.wapstore.pagina.total     = 1;
      this.wapstore.pagina.atual     = 1;
